...
begin
  ...
  -- Fallback: if no rows were captured because tx.order_id is NULL,
  -- infer the matching debits/credits by description containing "Order #<id>"
  -- and the companies linked to this order.
  IF NOT EXISTS (SELECT 1 FROM _del_need) THEN
    ...
    left join credit_transactions ct
     ...
     and (
          ct.description ilike 'Order #' || p_order_id || '%'
       or ct.description ilike '%Order #' || p_order_id || '%'
     )
  ...
  --------------------------------------------------------------------
  -- 4) DELETE all DEBIT rows for this order
  --------------------------------------------------------------------
