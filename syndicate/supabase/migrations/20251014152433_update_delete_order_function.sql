...
begin
  ...
  -- Update available + total balances (credit adds money back into the system)
  update company_credit_summary s
     set available_balance = round(s.available_balance + x.delta, 2),
         total_balance     = round(s.total_balance     + x.delta, 2),
         last_updated      = current_timestamp
    from (
