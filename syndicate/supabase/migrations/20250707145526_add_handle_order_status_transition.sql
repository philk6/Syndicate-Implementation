-- Migration: Add function to handle order status transitions
...
BEGIN
    ...
    IF p_new_status_id = v_closed_status_id AND p_old_status_id != v_closed_status_id THEN
        ...
        LOOP
            ...
            ELSE
                ...
                v_description := format('Order #%s closed - Actual cost debited ($%.2f of $%.2f max)', 
                    p_order_id, v_total_cost_value, company_rec.max_investment);
            END IF;
    ...
    ELSIF p_old_status_id = v_closed_status_id AND p_new_status_id != v_closed_status_id THEN
        -- Process each company that had allocations
        FOR company_rec IN
            SELECT oc.company_id, oc.max_investment, oc.allocated_amount
            FROM public.order_company oc
            WHERE oc.order_id = p_order_id
...
GRANT EXECUTE ON FUNCTION public.handle_order_status_transition(integer, integer, integer) TO authenticated;