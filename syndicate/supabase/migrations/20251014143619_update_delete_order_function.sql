...
begin
  ...
  -- 2) Compute per-company balancing amounts into a fresh temp table
  drop table if exists _del_need;
  create temporary table _del_need (
  ...
  -- 5) Delete dependents, then the order (all have WHERE clauses)
